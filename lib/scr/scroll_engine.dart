...
class ScrollLinker {
  ...
  double get offset {
    assert(
      ...
      'ScrollLinker does not have any scroll controllers '
      'attached.',
    );
...
/// This class provides change notification for [ScrollLinker]'s
/// scroll offset.
///
/// This change notifier de-duplicates change events by only firing listeners
/// when the scroll offset of the group has changed.
class _ScrollLinkerOffsetNotifier extends ChangeNotifier {
  _ScrollLinkerOffsetNotifier(this.controllerGroup);

  final ScrollLinker controllerGroup;
...
class _LinkedScrollController extends ScrollController {
  final ScrollLinker _controllers;
